...
for scenario in future_scenario_list:
    for model in model_list:
        ...
        maca_coll = ee.ImageCollection('IDAHO_EPSCOR/MACAv2_METDATA').filterDate(start_date, end_date)
        maca_coll = maca_coll.filterMetadata('model', 'equals', model).filterMetadata('scenario', 'equals', scenario)
        ...
        desc = f'{scenario}_{model}'
        export_name = f'maca/{desc}'
        # Export Summary Table
        ee.batch.Export.table.toCloudStorage(
            collection=ee.FeatureCollection(output.flatten()),
            selectors=['datenum', 'year', 'month', 'day', 'UACE10',
                       'tasmax', 'tasmin', 'rsds', 'uz', 'huss', 'pr', 'ws_2m', 'eto_mm', 'etr_mm'],
            description=desc,
